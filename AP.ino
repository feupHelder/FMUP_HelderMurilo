...
void handleData() {
  String send = "<!DOCTYPE html><html><body> <font size=\"1\"><button>Export HTML table to CSV file</button><p></p></font><form action=\"/back\"><button>Back</button></form>" + String(script) + "<table>";
  for (int i = 0; i < n; i++) {
    send = send + "<tr><th>" + String(i + 1) + "</th><th>" + data[i] + "</th></tr>";
  }
  send = send + "</table></body></html> ";
